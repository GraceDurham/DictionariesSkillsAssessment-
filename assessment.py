...
def get_melon_price(melon_name):
    ...
    melon_dictionary = {"Watermelon": 2.95, "Cantaloupe": 2.50, "Musk": 3.25, "Christmas": 14.25}

    if melon_name not in melon_dictionary:
...
def word_length_sorted(words):
    ...
    for word in words:
        ...
        if len_word not in word_length:
            word_length[len_word] = []

        word_length[len_word].append(word)

    word_length = word_length.items()

    for tup in word_length:
        word_value = tup[1]
        word_value.sort()
...
def translate_to_pirate_talk(phrase):
    ...
    for word in my_phrase:
        english_pirate = english_pirate + english_to_pirate.get(word, word) + " "
    
    english_pirate = english_pirate.strip()
    return english_pirate
